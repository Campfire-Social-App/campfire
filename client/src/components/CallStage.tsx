...
export function CallStage({ channelId, compact = false }: { channelId: string; compact?: boolean }) {
  ...
  return (
    <div className={cn(
      ...
      compact ? "mt-3 gap-3" : "flex-1 gap-5 p-6",
    )}>
        {participants.length === 0 ? (
          <div className="flex flex-col items-center gap-3 text-center">
            <Volume2 className="size-12 text-muted-foreground" />
            <p className="text-muted-foreground">{compact ? "Waiting for participants…" : "No one is in the voice channel."}</p>
            {!isThisChannelConnected && (
          ...
          <div className={cn("flex min-h-0 w-full flex-col gap-3", compact ? "h-[min(50vh,28rem)]" : "flex-1")}>
          ...
          <div
            className={cn("grid w-full max-w-6xl gap-3", compact && "mx-auto max-w-3xl")}
